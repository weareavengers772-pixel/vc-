...
function saveJSON(file, data) {
    try {
        ...
        console.error(`[VC+ SAVE ERROR] ${file}`, error);
    }
}
...
function deny(message) {
    return reply(
        message,
        box("You do not have permission to use this command.")
    );
}

function usage(message, text) {
    return reply(message, box("Usage: " + text));
}
...
const HELP_PAGES = [
...

    {
        ...
        description:
            ...
            "-vc kick @user\nDisconnect a member from your VC.\n\n" +
            "-vc ban @user\nBlock a member from the VC.\n\n" +
            "-vc permit @user\nAllow a member again.\n\n" +
            "-vc lock\nLock the VC.\n\n" +
            "-vc unlock\nUnlock the VC.\n\n" +
            "-vc chatlock\nLock VC chat so only the VC owner can type.\n\n" +
            "-vc chatunlock\nAllow members to type in VC chat again.\n\n" +
            "-vc limit 0-99\nSet the user limit.\n\n" +
            "-vc name <name>\nRename the VC.\n\n" +
            "-vc transfer @user\nTransfer ownership.\n\n" +
...
function helpPayload(page) {
    ...
    return {
        embeds: [
            new EmbedBuilder()
                .setTitle(`VC+ | ${p.name}`)
                .setDescription(box(p.description))
                .setFooter({
                    text: `Page ${safe + 1}/${HELP_PAGES.length}`
                })
        ],
        ...
        components: [
            new ActionRowBuilder().addComponents(
                ...
                new ButtonBuilder()
                    .setCustomId(`help_page_${safe}`)
                    .setLabel(String(safe + 1))
                    .setStyle(ButtonStyle.Secondary)
                    .setDisabled(true),
...
async function handleHelp(message, args) {
    if (args.length) {
        const page = HELP_PAGES.findIndex(
            p =>
                p.name.toLowerCase() ===
...
async function handleRank(message, args) {
    ...
    if (!RANKS[requested]) {
        return reply(message, box("Invalid rank."));
    }
    ...
    return reply(
        ...
        panel(
            "Rank",
            `${target.user.tag} is now **${RANK_DISPLAY[requested]}**.`
        )
    );
}
...
async function handleRemoveRank(message) {
    ...
    const oldRank =
        ranks[message.guild.id][target.id] ||
        "member";
    ...
    return reply(
        ...
        panel(
            "Remove Rank",
            `${target} was removed from **${RANK_DISPLAY[oldRank] || "Member"}** and returned to **Member**.`
        )
    );
}
...
async function handleVouch(message, args) {
    ...
    if (sub === "give") {
        ...
        if (target.user.bot) {
            return reply(
                message,
                box("Bots cannot receive vouches.")
            );
        }
    ...
    if (sub === "remove") {
        ...
        return reply(
            ...
            panel(
                "Vouch Remove",
                `Removed the latest vouch from ${target}.`
            )
        );
    }
                    ...
                    for (
                        ...
                        try {
                            await member.roles.remove(
                                role,
                                "VC+ vouches cleared for everyone"
                            );
...
async function handleVouches(message) {
    ...
    const history = list
        ...
        .map(
            (v, i) =>
                `${i + 1}. ${v.reason} — ${new Date(v.timestamp).toLocaleDateString()}`
        )
        .join("\n");
    ...
    return reply(
        ...
        panel(
            "Vouches",
            `${target.user.tag} has **${list.length}** vouch(es).\n\n${history}`
        )
    );
}
        ...
        if (command === "kick") {
            ...
            if (isFounder(target)) {
                return reply(
                    ...
                    panel(
                        "Kick",
                        "Founder members cannot be kicked by VC+."
                    )
                );
            }
...
function currentVC(member) {
    return member?.voice?.channel || null;
}

function isTempVC(c, id) {
    return c.temporaryChannels.includes(id);
}
...
function canControlVC(member, channel) {
    ...
    return (
        isGodOrHigher(member) ||
        c.voice.owners[channel.id] ===
            member.id
    );
}

async function createVCPanel(
    channel,
    owner
) {
    try {
        ...
        await channel.send({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(
                        `Owner: ${owner}\n\nUse the controls below or the -vc commands.`
                    )
                    .setFooter({
...
async function handleVC(
    ...
    if (sub === "setup") {
        ...
        try {
            ...
            return reply(
                ...
                panel(
                    "VC Setup",
                    `Join-to-Create created in **${category.name}**.`
                )
            );
        } catch (error) {
            console.error(
    ...
    try {
        if (
            ...
            if (isFounder(target)) {
                return reply(
                    ...
                    panel(
                        "Voice",
                        "Founder members cannot be kicked or disconnected."
                    )
                );
            }
        ...
        if (
            ...
            if (isFounder(target)) {
                return reply(
                    ...
                    panel(
                        "Voice",
                        "Founder members cannot be VC banned or rejected."
                    )
                );
            }
        ...
        if (
            ...
            // Only the VC owner can use chat lock/unlock.
            if (
                ownerId !==
                message.author.id
            ) {
            ...
            if (sub === "chatlock") {
                ...
                // Make sure the VC owner can still type.
                await channel.permissionOverwrites.edit(
                    message.member,
                    {
                        SendMessages: true,
                ...
                return reply(
                    ...
                    panel(
                        "Voice Chat",
                        "VC chat is now locked. Only the VC owner can type."
                    )
                );
            }
        ...
        if (
            ...
            if (sub === "stfu") {
                ...
                return reply(
                    ...
                    panel(
                        "Voice",
                        `${target.user.tag} is now STFU protected. The bot will automatically server mute them again if the mute is removed.`
                    )
                );
            }
            ...
            return reply(
                ...
                panel(
                    "Voice",
                    `${target.user.tag} is no longer STFU protected.`
                )
            );
        }
...
async function handleInterface(message) {
    ...
    try {
        let channel = c.interfaceChannel
            ? message.guild.channels.cache.get(
                  c.interfaceChannel
              )
            : null;
            ...
            await channel.send({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(
                            "Join your temporary VC and use the controls below. STFU is command-only for Founder, God and the server owner."
                        )
                        .setFooter({
                            text: "VC+ Voice Interface"
                        })
                ],
        ...
        return reply(
            ...
            panel(
                "Interface",
                `VC+ interface created in ${channel}.`
            )
        );
    } catch (error) {
        console.error(
...
async function handleInterfaceButton(
    interaction
) {
    const channel =
        getInteractionVC(
    ...
    if (
        ["limit", "rename"].includes(
            action
        )
    ) {
        return showValueModal(
            interaction,
            action
        );
    }
...
async function handleInterfaceModal(
    interaction
) {
    const channel =
        getInteractionVC(
    ...
    async (
        ...
        try {
            ...
            if (
                ...
                const channel =
                    await guild.channels.create(
                        {
                            name: `${newState.member.user.username}'s VC`.slice(
                                0,
                                100
                            ),
                            type: ChannelType.GuildVoice,
            ...
            if (
                ...
                delete c.voice.owners[
                    oldId
                ];

                delete c.voice.banned[
                    oldId
                ];

                delete c.voice.permitted[
                    oldId
                ];

                delete c.voice.locked[
                    oldId
                ];

                delete c.voice.chatLocked[
                    oldId
                ];

                delete c.voice.limits[
                    oldId
                ];
...
async function startBot() {
    ...
    if (!token) {
        console.error(
            "Missing DISCORD_TOKEN or TOKEN in .env"
        );
